...
def configure_ngrok():
    ngrok_token = "2pRdujkGtEZJ9tBjfLUDjA0YCej_4xAQj42qdBRouBWYbMZyM"
    try:
...
def update_soul_txt(public_url):
    with open("monikka.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in monikka.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("monikka.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated monikka.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update monikka.txt on VPS: {str(e)}")
